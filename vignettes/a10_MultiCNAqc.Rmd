...
Create a `m_CNAqc` object using the clonal CNAs. This will define new segments according to which genomic regions are affected by CNAs in all samples, and remap the mutations on them. Mutations that are not mapped on segments shared across samples can be accessed by the `mutations_on_private` attribute of the final object.

```{r}
example_multisample = multisample_init(cnaqc_objs = CNAqc_samples)
