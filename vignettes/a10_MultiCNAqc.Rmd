...
require(devtools)
require(tidyverse)
devtools::load_all(path = "C:/Users/lenovo/Documents/GitHub/CNAqc")
require(cli)
require(patchwork)
```
...
<!--# add cartoon with the new segmentation -->
...
```{r, echo=FALSE, dpi=300, fig.align='center', fig.width=2.5, fig.height=3}
chr1_plot = lapply(CNAqc_samples, function(x) {CNAqc::plot_segments(x, chromosomes = "chr1") +
...
The results will be stored as a list of CNAqc objects (one per sample) in the `cnaqc_obj_new_segmentation` attribute of the mCNAqc object. If running `multisample_init` with the `keep_original = TRUE`, the original CNAqc object will be stored in the `original_cnaqc_objc` attribute; else, results of any additional analysis performed on the original data (ie: peaks analysis or CCF estimation) will be stored in the `original_additional_info` attribute.
...
```{r}
# plot(example_multisample)
```

We can visualize the results of the new segmentation by using the `plot_segments` function:

```{r, echo = FALSE, dpi=300, fig.align='center', fig.width=2.5, fig.height=3}

new_seg = lapply(get_sample(example_multisample, which_obj = "shared"), function(x) {
  CNAqc::plot_segments(x, chromosomes = "chr1") +
    theme(axis.title = element_text(size = 4), 
        axis.text = element_text(size = 4), strip.text = element_text(size = 4), 
        plot.title = element_text(size = 4, hjust = 0.5), legend.text = element_text(size=6), 
        plot.caption = element_text(size = 4))})
