...
A Kernel-density method (unitary adjustment, Gaussian kernel) is used to smooth the data distribution, and the `peakPick` package to determine peaks in the smoothed density.  Peaks below a mnimum 
...
You can visually summarise the result of the QC analysis, which reports a small set of radio buttons (remote) for 

```{r, fig.width=3, fig.height=1, message=F, warning=F}
plot_qc(compute_CCF(x))
```
