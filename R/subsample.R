...
subset_by_minimum_CCF = function(x, min_target_CCF = 0.1)
{
  ...
  minor_copies = sapply(strsplit(used_karyotypes, split = ':'), function(x) x[2]) %>% as.numeric()
  major_copies = sapply(strsplit(used_karyotypes, split = ':'), function(x) x[1]) %>% as.numeric()
  ...
  cuts_table = data.frame(karyotype = used_karyotypes, min_target_CCF = min_target_CCF, VAF_cutoff = cutoffs, stringsAsFactors = FALSE)
  ...
  if(nrow(will_not_affect) > 0){
    cat("\n")
    cli::cli_alert_warning("Some karyotypes will not be affected by the desired cut: {.field {will_not_affect$karyotype}}")
  }
...
subset_snvs = function(x, ref_nucleotides = c("A", "C", "T", "G"), alt_nucleotides = c("A", "C", "T", "G"))
{
  ...
  {  
    cli::cli_alert_success("Retained {.field {nrow(subset_snvs)}} SNVs, removed {.field {nrow(removed_snvs)}}.")
