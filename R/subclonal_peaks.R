                                   ...
                                   kernel_adjust = 1)
{
  ...
  subclonal_mutations = lapply(1:nrow(subclonal_calls),
                               function(i)
                               {
                                 ...
                                 L = round((subclonal_calls$to[i] - subclonal_calls$from[i])/1e6, 1)
  ...
  data_fit = easypar::run(
    FUN = function(i) {
      mutations = subclonal_calls$mutations[[i]]

      # Get default: all data run
      s_run = simple_peak_detector(mutations, kernel_adjust = kernel_adjust)
      ...
      if (n_bootstrap > 1)
      {
        ...
        sb_run = lapply(1:n_bootstrap, function(e) {
          simple_peak_detector(mutations %>% sample_n(N, replace = TRUE),
...
#
# x = init(snvs = x$mutations, cna = x$cna, purity = x$metadata$purity, ref = 'hg19')


## x = readRDS("~/Downloads/f48c3c82-bebe-4b8e-909e-e1a51a7142ec.rds")# x = init(x$mutations, x$cna, x$metadata$purity)# x = x %>% analyze_peaks(n_bootstrap = 10)# plot_peaks_analysis(x, what = 'general')
