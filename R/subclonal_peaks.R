...
subclonal_peak_detector_closest_hit_match = function(
  chr,
  from,
  to,
  mutations,
...
)
{
  ...
  # Filter unused peaks
  if(karyotype_1 %in% c("1:0", "1:1")) peak_2_1 = NA
  if(karyotype_1 %in% c("1:0", "1:1")) peak_2_2 = NA
  ...
  segment_plot =
    ...
    labs(
      ...
      subtitle = paste0(
        karyotype_1, ' (', CCF_1 %>% round(2), ') - ',
        karyotype_2, ' (', CCF_2 %>% round(2), ') '
      )
    )
...
analyse_subclonal_peaks = function(x, mutations, subclonal_CNAs, epsilon)
{
  ...
  req_names = c("chr", "from", "to", "CCF_1", "CCF_2", "Major_1", "minor_1", "Major_2", "minor_2")
  missing_names = setdiff(req_names, colnames(subclonal_CNAs))

  if(length(missing_names) > 0)
  {
    cli::cli_alert_danger("The following columns are missing from your subclonal CNAs table: {.field {missing_names}}.")
    return(x)
  }
  ...
  if(length(missing_names) > 0)
  {
    cli::cli_alert_danger("The following columns are missing from your mutations table: {.field {missing_names}}.")
    return(x)
  }
  ...
  subclonal_CNAs = subclonal_CNAs %>%
    mutate(
      ...
      analysed =
        (karyotype_1 %in% supported_peaks) & (karyotype_2 %in% supported_peaks)
      )

  if(any(!subclonal_CNAs$analysed))
  {
    cli::cli_alert_warning("The following segments have subclonal CNAs that are not supported by CNAqc and will be rejected.")
  ...
  for(i in 1:nrow(subclonal_CNAs))
  {
    subclonal_CNAs$mutations[i] = mutations %>%
      dplyr::filter(
        chr == subclonal_CNAs$chr[i],
        from  >= subclonal_CNAs$from[i],
        to <= subclonal_CNAs$to[i]
      ) %>% list()
  }
  ...
  if(any(sapply(subclonal_CNAs$mutations, nrow) >= 1))
  {
    ...
    subclonal_CNAs = subclonal_CNAs[sapply(subclonal_CNAs$mutations, nrow) >1, ]
  }
  ...
  tab_heatmap = ggplot(
    ...
    scale_fill_manual(
      values = c(`TRUE` = 'forestgreen', `FALSE` = 'indianred', `NA` = 'gray')
    ) +
    facet_wrap(~clone) +
    CNAqc:::my_ggplot_theme() +
  ...
  size_barplot = all_fits_table %>%
    ggplot() +
    geom_bar(aes(x = id, fill = clone, CCF), stat = 'identity', width = .8) +
    coord_flip()  +
  ...
  x$subclonal = list(
    figure = list(figure = figure, fits = all_fits_plots),
...
parse_Battenberg = function(x)
{
  required = c(
    "chr", "from", "to",
    "battenberg_nMaj1_A", "battenberg_nMin1_A",
    "battenberg_nMaj2_A", "battenberg_nMin2_A",
    "battenberg_frac1_A", "battenberg_frac2_A"
    )

  if(!all(required %in% colnames(x)))
    stop(cli::format_error("Missing Battenberg columns in these calls! Required columns are: {.field {required}}"))
  ...
  clonal_calls = all_calls %>%
    ...
    dplyr::rename(Major = Major_1, minor = minor_1, CCF = CCF_1) %>%
