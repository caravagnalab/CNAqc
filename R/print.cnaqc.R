...
print.cnaqc = function(x, ...)
{
  ...
  if (with_peaks)
  {
    ...
    if (x$peaks_analysis$QC == "PASS")
      cli::cli_h3(
        paste0(ppass()," Peaks QC {crayon::bold(x$peaks_analysis$matching_strategy)}: % {crayon::green(paste0(prop))} with {crayon::green(paste('q =', x$peaks_analysis$score))}"
      ))
    else
      cli::cli_h3(
        paste0(pfail(), " Peaks QC {crayon::bold(x$peaks_analysis$matching_strategy)}: % {crayon::red(paste0(prop))} with {crayon::red(paste('q =', x$peaks_analysis$score))}"
      ))
    ...
    for(karyo in xx$karyotype %>% unique)
    {
      ...
      cli::cli_alert_info(
        paste0(crayon::blue(xxx$karyotype[1])," ~ n = {n} ({p}%) {clisymbols::symbol$arrow_right} ", qc, "")
      )
